...
const minDeletionSize = function(A) {
    let res = 0, n = A.length, m = A[0].length, i, j; //n: 有多少个字符串，对应i.  m:每个字符串的长度,对应j
    let sorted = new Array(n - 1).fill(false);
    for (j = 0; j < m; ++j) { //从第一个字符到最后一个字符
        for (i = 0; i < n - 1; ++i) { //i从第一个字到最后一个字
        ...
        for (i = 0; i < n - 1; ++i) //这一段代码结合最外面的循环可以用作比较string大小的通用代码
